...
| 2026-04-15 14:15 | analyse last run log files to identify what whent wrong | Analyzed logs: agent ran OK but config issue - file_sensor watching entire user home (474k files). Fixed: changed WATCH_DIRECTORY to project folder, added .logs/ to gitignore. |
| 2026-04-15 14:25 | in current TUI there are no place show what hapening on background. add this feature couse when agent or other task runing on background as a user I want to monitor them | Added BACKGROUND TASKS panel to dashboard showing: active/pending threadpool tasks, errors, debug logs. Committed. |
